...
void CCMainWindow::setSignName(const QString & signName)
{
	ui.lineEdit->adjustSize();

	//文本过长则进行省略...
	//fontMetrics()返回QFontMetrics类对象
	QString name = ui.nameLabel->fontMetrics().elidedText(signName, Qt::ElideRight, ui.lineEdit->width());
...
void CCMainWindow::initContactree()
{
	...
	QString strGroupName = QString::fromLocal8Bit("SQH自制版QtQQ");
...
{
	//setUserName(QString::fromLocal8Bit("越分享越拥有"));
	QSqlQuery sqlQuery(QString("SELECT employee_name,employee_sign FROM tab_employees WHERE employeeID=%1").arg(gLoginEmployeeID.toInt()));
	sqlQuery.exec();
	if (sqlQuery.first()) {
		QString signName = sqlQuery.value(1).toString();
